...
export default function Detail() {
  ...
  async function handlePickStream(stream: any) {
    ...
    try {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (window as any).__lebaCancelPlay = false;
      const result = await playTorrent(stream.infoHash, stream.name, pickerSeason, pickerEpisode, stream.fileIdx);
      if ((window as any).__lebaCancelPlay) { (window as any).__lebaCancelPlay = false; setPlayState(null); return; }
      if (isRemote) {
                      ...
                      <div className="episode-actions">
                        <button
                          ...
                          onClick={() => {
                            ...
                            const epProg = episodeProgress.get(epKey);
                            const nowFinished = !epProg?.finished;
                            reportWatchProgress({
                              tmdbId: Number(id),
                              mediaType: "tv",
                              ...
                              setEpisodeProgress((prev) => {
                                ...
                                next.set(epKey, { ...epProg, position: nowFinished ? dur : 0, duration: dur, finished: nowFinished });
                                return next;
                              });
                              fetchResumePoint(Number(id), type).then((r) => {
                                setResumePoint(r.resumePoint);
