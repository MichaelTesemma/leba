...
export function PlayerProvider({ children }: PlayerProviderProps) {
  ...
  // Sync React state from effectiveTimeRef (updated by Player.tsx)
  useEffect(() => {
  ...
  const startStream = useCallback((infoHash: string | number, fileIndex: string | number, title: string, tags: string[], debridStreamKey?: string) => {
    ...
    // Save position of the current stream before switching
    if (active) {
  ...
  // togglePlay is implemented by the Player component (Artplayer)
  // and registered via commandRef.seek / commandRef.seekRelative
  const togglePlay = useCallback(() => {
    // Delegate to the active player via commandRef
    // Artplayer exposes play/pause via its own API; Player.tsx handles this
    if (commandRef.current) {
      // Player.tsx will override this — noop here, actual impl in Player
    }
  }, []);
        ...
        case "start-stream":
          if (value) {
            ...
            if (wasOnPlayer) {
              ...
              // Small delay to let player unmount before starting new stream
              setTimeout(() => {
  ...
  useEffect(() => {
    ...
    function reportState() {
      ...
      // Fast path: skip serialization if only time changed by < 1s and nothing else changed
      if (
        playing === lastPlaying &&
        Math.abs(ct - lastCt) < 1 &&
        dur === lastDur &&
        !active && !switchingRef.current
      ) return;
      lastPlaying = playing;
      lastCt = ct;
      lastDur = dur;
  ...
  }, [rcAuthToken, rcSessionId, active, playing]);
