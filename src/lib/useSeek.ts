...
interface UseSeekDeps {
  ...
  seekFn?: (seconds: number) => void;
  onTorrentNotFound?: () => void;
}
...
export function useSeek(deps: UseSeekDeps): UseSeekReturn {
  const {
    ...
    seekRef, seekFn, onTorrentNotFound,
  } = deps;
  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        ...
        if (stopped) return;
        // Torrent not on server — stop polling permanently
        if (data.error) { stopped = true; onTorrentNotFound?.(); return; }
