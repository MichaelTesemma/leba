...
export function useSubtitles(deps: UseSubtitlesDeps): UseSubtitlesReturn {
  ...
  useEffect(() => {
    ...
    loadSubs();
    timer = setInterval(loadSubs, 5000);
    ...
    async function loadSubs() {
      try {
        ...
        if (data.tracks?.length > 0) {
          ...
          retryCount++;
          if (retryCount >= MAX_RETRIES && timer) {
            clearInterval(timer);
          }
        }
      } catch {
  ...
  useEffect(() => {
    fetchStatus(infoHash).then((data: { files?: Array<{ index: number; path?: string; name?: string; isSubtitle?: boolean }>; error?: string }) => {
