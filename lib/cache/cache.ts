...
/**
 * LRU-aware TTL cache. Uses Map insertion-order to evict oldest entries
 * on overflow (O(1) vs the previous O(n log n) sort-based eviction).
 */
export class TTLCache {
  ...
  get(key: string): unknown {
    ...
    // Move to end (most recently used) — LRU semantics
  ...
  set(key: string, value: unknown, ttl?: number): void {
    // Delete first so existing key moves to end (MRU position)
  ...
  private _evict(): void {
    // Evict oldest entries from the front — Map preserves insertion order
    // O(n) vs the old O(n log n) sort
    const count = Math.ceil(this._maxEntries * EVICT_RATIO);
    const keys = this._map.keys();
    for (let i = 0; i < count; i++) {
      const key = keys.next().value;
      if (key !== undefined) this._map.delete(key);
    }
  }
}
