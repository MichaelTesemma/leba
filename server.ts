...
app.use(compression()); // gzip/brotli compression — reduces JSON payloads by 60-80%
...
app.get("/{*splat}", (_req: Request, res: Response) => {
  ...
  return {
    app, get client() { return ctx.client; },
    durationCache, seekIndexCache, seekIndexPending,
...
if (isMain) {
  const ctx = createApp();
  const { app, client, activeTranscodes } = ctx;
  ...
  app.listen(PORT, HOST, () => {
    console.log(`[${new Date().toISOString().slice(11, 23)}] INFO  Leba running at http://${HOST}:${PORT}`);
