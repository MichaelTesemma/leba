  ...
  app.post("/api/vpn/toggle", async (req: Request, res: Response) => {
    ...
    if (ppid && ppid > 1) {
      try {
        ...
        res.status(500).json({ error: "Failed to signal supervisor. Is leba-vpn running?" });
      }
    } else {
      log("warn", "No supervisor detected (ppid=1). VPN toggle requires the leba-vpn supervisor.");
      res.status(400).json({ error: "VPN toggle requires the leba-vpn supervisor. Start with ./leba-vpn instead of node server.ts" });
    }
  });
