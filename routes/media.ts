...
export default function mediaRoutes(app: Express, ctx: ClientCtx & CacheCtx & LogCtx & DebridCtx): void {
  ...
  // Pending probes — prevent spawning multiple ffprobe for the same file
  const pendingProbes = new Map<string, Promise<number | null>>();
  ...
  app.get("/api/duration/:infoHash/:fileIndex", (req: Request, res: Response) => {
    ...
    // If already probing, wait for the in-flight result
    if (pendingProbes.has(cacheKey)) {
      return pendingProbes.get(cacheKey)!.then((dur) => {
        if (dur) res.json({ duration: dur });
        else res.json({ duration: null });
      });
    }
    ...
    const probePromise = new Promise<number | null>((resolve) => {
